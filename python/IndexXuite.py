...
for craw in crawler:
    ...
    print(craw)
    url='http://www.a.blog.xuite.net/new_index.php?type=put_list_roller&tag_id='+(url_list[craw])
    print(url)
    ...
    for imgs in soup.select('.p1'):
        ...
        cur.execute(sqli,(tag_list[craw],search_title,search_author,search_href,article_picture)) #存入資料庫    
        conn.commit()
